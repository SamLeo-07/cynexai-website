...
const Footer = () => {
  ...
  const socialLinks = [
    { icon: Facebook, href: '#', hoverColor: 'hover:text-[#376db0]' },
    { icon: Twitter, href: '#', hoverColor: 'hover:text-[#376db0]' },
    { icon: Instagram, href: '#', hoverColor: 'hover:text-[#376db0]' },
    { icon: Linkedin, href: '#', hoverColor: 'hover:text-[#376db0]' },
    { icon: Youtube, href: '#', hoverColor: 'hover:text-[#376db0]' },
  ];
  ...
  return (
    ...
    <footer className="relative bg-white border-t border-gray-200 text-gray-700 overflow-hidden"> {/* Added overflow-hidden here for the extra black space issue */}
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-0 left-1/4 w-64 h-64 bg-[#376db0]/5 rounded-full blur-3xl"></div>
        <div className="absolute bottom-0 right-1/4 w-80 h-80 bg-[#376db0]/5 rounded-full blur-3xl"></div>
      </div>
            ...
            <div className="lg:col-span-2">
              ...
              <div className="space-y-3">
                <div className="flex items-center text-gray-600">
                  <Phone className="w-4 h-4 mr-3 text-[#376db0]" />
                  <span>+91 9966639869</span>
                </div>
                <div className="flex items-center text-gray-600">
                  <Mail className="w-4 h-4 mr-3 text-[#376db0]" />
                  <span>contact@Cynexai.in</span>
                </div>
                <div className="flex items-start text-gray-600">
                  <MapPin className="w-4 h-4 mr-3 mt-1 text-[#376db0] flex-shrink-0" />
              ...
              <ul className="space-y-3">
                {footerLinks.courses.map((link, index) => (
                  <li key={index}>
                    <Link
                      to={link.href}
                      className="text-gray-600 hover:text-[#376db0] transition-colors duration-200 text-sm"
                    >
                      {link.name}
                    </Link>
                  </li>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4"> {/* Reduced gap slightly for mobile */}
            ...
            <div className="flex w-full md:w-auto flex-nowrap">
              <input
                ...
                className="flex-1 min-w-0 px-4 py-3 bg-gray-100 border border-gray-300 rounded-l-lg text-black placeholder-gray-500 focus:outline-none focus:border-[#376db0] focus:ring-2 focus:ring-[#376db0]/20" // Input takes available space, allows shrinking
              />
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="px-6 py-3 bg-[#376db0] text-black hover:bg-yellow-600 rounded-r-lg font-medium transition-all duration-300 flex-shrink-0" // Add flex-shrink-0 to button
              >
                Subscribe
              </motion.button>
            </div>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.9 }}
        className="fixed bottom-4 right-4 z-50 bg-[#376db0] text-black p-3 rounded-full shadow-lg hover:bg-yellow-600 transition-colors duration-300"
      >
