      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div ref={ref} variants={containerVariants} initial="hidden" animate={inView ? 'visible' : 'hidden'} className="text-center mb-16">
          <motion.h2 variants={itemVariants} className="text-4xl md:text-5xl font-display font-bold mb-6">
            <span className="text-[#300A6E]">Transform Your Skills</span>
          </motion.h2>
        ...
        <motion.div variants={containerVariants} initial="hidden" animate={inView ? 'visible' : 'hidden'} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {courses.map((course, index) => (
            <motion.div key={course.id} variants={itemVariants} whileHover={{ y: -10, scale: 1.02 }} className="group bg-gray-50 rounded-2xl overflow-hidden border border-gray-200 hover:border-[#300A6E] transition-all duration-300 shadow-sm">
              ...
              <div className="p-6">
                ...
                <div className="flex items-center justify-between mb-4 text-sm text-gray-600">
                  ...
                  <div className="flex items-center">
                    <Star className="w-4 h-4 mr-1 text-[#300A6E]" />{course.rating}
                  </div>
                </div>

                <div className="flex flex-wrap gap-2 mb-6">
                  {course.skills.slice(0, 3).map((skill, skillIndex) => (
                    <span key={skillIndex} className="px-2 py-1 bg-[#300A6E]/10 text-[#300A6E] rounded-md text-xs border border-[#300A6E]/30">
                      {skill}
                    </span>
                  ))}
                ...
                <Link to={`/course/${course.id}`} className="w-full block bg-[#300A6E] text-white py-3 px-4 rounded-lg font-medium text-center hover:bg-[#c09a2f] transition-colors duration-300 flex items-center justify-center">
                  Learn More
