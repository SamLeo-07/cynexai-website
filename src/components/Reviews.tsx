  ...
  return (
    <section id="reviews" className="py-20 relative overflow-hidden bg-white"> {/* Changed background to white */}
      ...
      <div className="absolute inset-0">
        <div className="absolute top-20 left-10 w-72 h-72 bg-[#72b8f8]/10 rounded-full blur-3xl animate-pulse-slow"></div> {/* Gold tint */}
        <div className="absolute bottom-20 right-10 w-96 h-96 bg-[#72b8f8]/10 rounded-full blur-3xl animate-pulse-slow" style={{ animationDelay: '2s' }}></div> {/* Gold tint */}
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          >
            <span className="text-[#72b8f8]"> {/* Changed to gold text */}
          ...
          <div className="relative max-w-4xl mx-auto">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="bg-white rounded-3xl p-8 md:p-12 border border-[#72b8f8]/30 shadow-xl transition-all duration-300" // Changed background to white, border to gold
              >
                <div className="flex flex-col md:flex-row items-center gap-8">
                  {/* Profile Image */}
                  <div className="relative flex-shrink-0">
                    <div className="w-24 h-24 md:w-32 md:h-32 rounded-full overflow-hidden border-4 border-[#72b8f8] p-1"> {/* Solid gold border */}
                      <img
                    ...
                    <div className="absolute -top-2 -right-2 bg-[#72b8f8] rounded-full p-2"> {/* Gold quote bubble */}
                  ...
                  <div className="flex-1 text-center md:text-left">
                    ...
                    <div>
                      ...
                      <p className="text-[#72b8f8] font-medium mb-1"> {/* Changed text to gold */}
            ...
            <button
              onClick={prevReview}
              className="absolute left-4 top-1/2 transform -translate-y-1/2 bg-[#72b8f8]/10 hover:bg-[#72b8f8]/20 backdrop-blur-sm rounded-full p-3 border border-[#72b8f8]/20 hover:border-[#72b8f8]/50 transition-all duration-300 group" // Gold tint/border
            >
              <ChevronLeft className="w-6 h-6 text-[#72b8f8] group-hover:text-white" /> {/* Gold icon, white on hover */}
            </button>
            
            <button
              onClick={nextReview}
              className="absolute right-4 top-1/2 transform -translate-y-1/2 bg-[#72b8f8]/10 hover:bg-[#72b8f8]/20 backdrop-blur-sm rounded-full p-3 border border-[#72b8f8]/20 hover:border-[#72b8f8]/50 transition-all duration-300 group" // Gold tint/border
            >
              <ChevronRight className="w-6 h-6 text-[#72b8f8] group-hover:text-white" /> {/* Gold icon, white on hover */}
            </button>
          </div>
            ...
            {reviews.map((_, index) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all duration-300 ${
                  index === currentReview
                    ? 'bg-[#72b8f8] scale-125' // Solid gold when active
            ...
            {reviews.map((review, index) => (
              <motion.div
                ...
                className={`cursor-pointer bg-white rounded-xl p-4 border transition-all duration-300 shadow-sm ${ // White background
                  index === currentReview
                    ? 'border-[#72b8f8] bg-[#72b8f8]/10' // Gold border/tint when active
              ...
              >
                <div className="flex items-center space-x-3 mb-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#72b8f8]">{review.role}</p> {/* Gold text */}
                  </div>
                </div>
